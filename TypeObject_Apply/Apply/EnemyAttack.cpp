...
EnemyAttack* EnemyAttack::initEnemyAttack(Enemy *enemy, CCPoint point)
{
	...
	this->initWithFile(initFileName(enemy->getEnemyTypeTag()).c_str());
	this->setPosition(point);
	
	return this;
}

std::string EnemyAttack::initFileName(enemyTypeTag type)
{
	std::string fileName;
	
	switch (type)
	{
	case enemyTypeTag::kTag_rat1:
	case enemyTypeTag::kTag_rat2:
		fileName = "attack_rat1.png";
		break;
	case enemyTypeTag::kTag_vampire:
